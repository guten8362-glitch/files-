...
export default async ({ req, res, log, error }) => {
  ...
  // Use a Provider ID from env or a common placeholder if not set
  ...
  if (event) {
    ...
    try {
      ...
      const employeeId = payload.employee_one || ''; // Checking assigned employee email/ID

      const messageContent = `${errorType} detected on ${printerId}`;
      
      if (employeeId) {
        // Direct Push to assigned technician
        log(`Sending direct push to ${employeeId}`);
        await messaging.createPush(
          ID.unique(),
          'SupportA4 Alert',
          messageContent,
          [employeeId] // Appwrite uses userId (which might be the email in this project)
        );
      } else {
  ...
  try {
    ...
    if (path === '/tasks' && method === 'GET') {
      const tasks = await databases.listDocuments(DATABASE_ID, COLLECTION_MAINTENANCE, [
        Query.orderAsc('startTime'), // Oldest first (within priority)
        Query.limit(100)
      ]);
    ...
    if (path === '/saveToken' && method === 'POST') {
      ...
      // Register device as a Target for Messaging API
